...
interface Props { onComplete: () => void; pageCompleted: boolean; onNext: () => void }

export default function EmailUrlClick({ onComplete, pageCompleted, onNext }: Props) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-44 border-r border-[var(--color-border)] p-3 shrink-0 overflow-auto">
          <div className="space-y-0.5">
            {FOLDERS.map(f => (
              <div key={f.name} className="flex items-center justify-between px-2 py-1.5 rounded text-sm">
          ...
          <div className="border-t border-[var(--color-border)] mt-3 pt-3 space-y-0.5">
            {LABELS.map(l => (
              <div key={l} className="px-2 py-1.5 text-sm text-[var(--color-text-secondary)]">
                {l}
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-auto">
          ...
          <div className="px-8 py-6 space-y-4 text-sm text-[var(--color-text-secondary)] leading-relaxed">
            <p>Hi Priya,</p>
            <p>I took a first pass at testing VoltMart's latest release and found something interesting at the product web page.</p>
            <p>It's best you take a look at it for yourself,</p>
            <p>Regards,</p>
            <p>Kai</p>

            {/* Clickable malicious URL with highlight */}
            <div className="mt-6 pt-4 border-t border-[var(--color-border)]">
              <button
                id="malicious-url-btn"
                onClick={handleUrlClick}
                disabled={clicked}
                className={`text-left text-blue-600 hover:text-blue-700 text-sm break-all underline font-mono leading-relaxed transition-all relative ${
                  showHighlight ? 'bg-amber-100 rounded px-2 py-1 border border-amber-300 animate-pulse-glow' : ''
              ...
              >
                ...
                {showHighlight && (
                  <span className="absolute -top-7 left-0 bg-amber-600 text-white text-[10px] font-semibold px-2 py-0.5 rounded whitespace-nowrap animate-fade-in">
                    ← Click this URL
                  </span>
                )}
              </button>
            ...
            {clicked && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 animate-fade-in mt-3">
                <p className="text-xs text-red-700 font-medium">
                  ⚠️ Priya clicked the link — her browser is now loading the VoltMart page with the malicious payload...
                </p>
              </div>
            )}
          </div>
